...
public class PosController {
    ...
    @GetMapping("/mon-an")
    public ApiResponse<List<com.restaurant.entity.MonAn>> getMonAn(
            @RequestParam(required = false) String category,
            @RequestParam(required = false) String keyword
    ) {
        List<com.restaurant.entity.MonAn> list;

        if (category != null && keyword != null) {
            list = service.findByCategoryAndKeyword(category, keyword);
        } else if (category != null) {
            list = service.findByCategory(category);
        } else if (keyword != null) {
            list = service.searchByName(keyword);
        } else {
            list = service.getAllMonAn();
        }

        return ApiResponse.<List<com.restaurant.entity.MonAn>>builder()
                .code(ErrorCode.SUCCESS.getCode())
                .result(list)
                .build();
    }
